...
#sampleTable_2h <- sampleTable %>% filter(time == "2h")
...
###### rpfB 0 vs 2 ######
ma_rpfB <- as.data.frame(results(dds_trunc, contrast = c("type","304","304-SW"), cooksCutoff = F))
...
# RNA_targets <- read.csv(paste(here::here("PR1048/targetRNA3.csv"))) %>%
#   mutate(real_name = ifelse(grepl("\\(MSMEG",Target),
#                             sub(".\\(.*","\\1",Target),
#                             gff2$realID[match(Target,gff2$weird_locus)]))
# 
# hits0 <- RNA_targets %>%
#   filter(real_name %in% rownames(LFC_0h_table)) %>%
#   mutate(foldchange = LFC_0h_table$log2FoldChange[match(real_name,rownames(LFC_0h_table))],
#          change_p = LFC_0h_table$padj[match(real_name,rownames(LFC_0h_table))])
# 
# hits_sig_0 <- hits0 %>%
...
#                 "sRNA-probability" = 6)
# 
# hits_compare <- RNA_targets %>%
#   filter(real_name %in% rownames(LFC_compare_table)) %>%
#   mutate(foldchange = LFC_compare_table$log2FoldChange[match(real_name,rownames(LFC_compare_table))],
#          change_p = LFC_compare_table$padj[match(real_name,rownames(LFC_compare_table))])
# 
# hits_sig_compare <- hits_compare %>%
#   filter(change_p < 0.05) %>%
#   select(12,10,13,14,4,5) %>%
#   dplyr::rename("locus" = 1,
#                 "RNAseq-change" = 3,
...
#                 "sRNA-probability" = 6)
# 
# fwrite(hits_sig_0, file=paste0(directory,"/0h_sRNA_hits.csv"), row.names = T)
